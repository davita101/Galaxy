    ...
    return (
        <div className='flex items-center justify-between lg:pl-[1rem] lg:pt-[2.5rem]  pl-[1rem] pt-[2.5rem] fixed w-full z-[99]'>
                ...
                <ul className='flex gap-[3.12rem] ml-[2rem] p-[3rem] pb-0 bg--color lg:pr-[10rem]  lg:pl-[12rem]  backdrop-blur-md  z-[3] relative border-white border-opacity-[0.01] items-center justify-end'>
                    {navbar.map((item, index) => (
                        <Link key={index} to={item.link}>
                            <div onClick={() => hendleActive(index)} className='sm:flex hidden flex-col justify-between items-end gap-[0.68rem] h-[4rem] subheading-fs--300 cursor-pointer z-[3] text-white'>
            ...
            <div className="sm:hidden flex fixed right-0 top-0 ">
                <ul className={`${Click ? "translate-x-[250px]" : "translate-x-0"} transition-all flex gap-[2rem] flex-col pb-[7rem] bg--color pl-[3rem] relative lg:pl-[12rem]  backdrop-blur-md  w-[15rem] h-[100vh] z-[3] border-white border-opacity-[0.01] items-start justify-center `}>
                    {navbar.map((item, index) => (
                        <Link key={index} to={item.link}>
                            <div onClick={() => { hendleActive(index), setClick(true) }} className='flex flex-row w-[12rem] justify-between gap-[0.68rem] subheading--300 cursor-pointer z-[3] text-white'>
                                <div className='flex gap-[0.68rem]'>
                                    <span className='font-bold'>{item.num}</span>
                                    <li>{item.li}</li>
                                </div>
